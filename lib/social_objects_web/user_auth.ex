defmodule SocialObjectsWeb.UserAuth do
  @moduledoc """
  Authentication helpers for browser sessions and LiveView mounts.
  """
  ...
  @remember_me_cookie "_social_objects_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  defp renew_session(conn, %Accounts.User{} = user) do
    case conn.assigns[:current_scope] do
      %Scope{user: %Accounts.User{id: id}} when id == user.id -> conn
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      SocialObjectsWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule SocialObjectsWeb.PageLive do
        use SocialObjectsWeb, :live_view

        on_mount {SocialObjectsWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SocialObjectsWeb.UserAuth, :require_authenticated}] do
  ...
  # the user was already logged in, redirect to app root
